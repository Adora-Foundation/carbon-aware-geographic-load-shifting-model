...
effect_of_load lsm_commercial =
    let
        ...
        lsms = [
                ...
                \lambda -> lsm_commercial{lambda_hi=lambda/n_steps,lambda_lo=lambda/n_steps,c_em=0,alpha=alpha_opt lambda},
...
main = do
    mapM putStrLn $ createCSV $ createTableLoad $ foldl (++) [] $ map  effect_of_load $ doeCommercial++doeHPC
